...
class GatherInformation(Workflow):
    ...
    async def parse_userQuery(self, ctx: Context, ev: ParseInput_Event) -> QueryGeneration_Event:
        ...
        """
        You are a helpful flight attendant. If you encounter time-related terms like 'yesterday','tomorrow','today' or a specifice date, just extract as it is into departure time or arrival_time. 
        Extract the following information from the query below:
        - flight_id
        - departure_time
        - departure_airport
        - arrival_airport
        ...
        try:
            ...
            extracted_input = llm.structured_predict( DatabaseSchema, simplified_prompt, text= ev.payload)
    ...
    async def postprocess_query(self, ctx: Context, ev: PostProcessQuery_Event) -> QueryGenerationComplete_Event:
        ...
        '''
        departTime = tmp.get('departure_time', None)  # Default to None if the key doesn't exist
        # arrivalTime = tmp.get('arrival_time', None)
        ...
        #     tmp['arrival_time'] = parsedArrival
